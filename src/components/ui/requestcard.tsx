...
export default function RequestCard({
  ...
  const [completedCount] = useState<number>(3);
  const [pendingCount] = useState<number>(5);
  const [cancelledCount] = useState<number>(6);
    ...
    <Card className="h-32 shadow-md rounded-2xl bg-[#f5e9e8] w-full mx-auto">
      <CardHeader className="h-full gap-4 flex flex-row items-center justify-center text-black text-lg font-medium">
        <div className="flex items-center gap-2">
          <IconTruck className="w-16 h-16 sm:w-20 sm:h-20" />
          <span
            className={cn(
              "font-semibold",
        ...
        <span className="hidden xl:inline-block text-base xl:text-lg">
          {title}
        </span>
      </CardHeader>
    </Card>
